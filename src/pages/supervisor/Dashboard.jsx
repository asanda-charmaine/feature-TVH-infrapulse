...
import { useStore, validateSupervisorReport, assignSupervisorTask, reviewSupervisorCompletion, dismissSupervisorReport } from '../../lib/store.js';
import { supervisorQueues, suggestedTechnicians, validationReasons, isSupervisorValidated } from '../../lib/supervisor.js';
import { CATEGORIES, SEVERITIES } from '../../lib/constants.js';
import { toDateInput, fmtDateTime } from '../../lib/format.js';

const tabs = [['incoming', 'Incoming Tasks'], ['validated', 'Validated Tasks'], ['assigned', 'Assigned Tasks'], ['active', 'In Progress'], ['completed', 'Completed Tasks']];
function Decision({ entry, state, onClose, view }) {
  ...
  const needsValidation = view === 'review' && !report.supervisorDismissal && report.status !== 'Resolved' && !isSupervisorValidated(report);
  const needsReview = task?.status === 'Completed' && task.supervisorReview?.decision !== 'accepted';
  ...
  return <Modal title={task?.id || report.id} onClose={onClose}>
    <div className="stack">
      <section><h3>Problem</h3><div className="btn-row"><SourceBadge source={report.source} /><StatusBadge status={task?.status || report.status} /></div><p className="muted small">{report.id} · {fmtDateTime(report.submittedAt)}</p><p className="muted small">Risk: {task?.risk ?? report.risk}</p><p><strong>{task?.issue || report.issue || report.ai?.detected || report.category}</strong></p><p>{report.location.address}</p><SeverityBadge severity={task?.priority || report.severity} /></section>
      <section><h3>System / AI Result</h3><p>{report.ai?.detected || 'No classification available'}{report.ai?.confidence != null ? ' · ' + report.ai.confidence + '% confidence' : ''}</p><p className="muted small">{needsValidation ? (validationReasons(report).join(' · ') || 'Awaiting supervisor review') : report.supervisorValidation ? 'Supervisor decision recorded' : 'No validation decision needed'}</p>
        {report.image && <details><summary>View supporting image</summary><Photo src={report.image} alt="Reported infrastructure issue" /></details>}
      </section>
      ...
      <section className="stack-sm"><h3>Supervisor Decision</h3>
        {(needsValidation || needsReview) && <div className="field"><label htmlFor="decision-note">Short comment (optional)</label><input id="decision-note" className="input" maxLength={300} value={note} onChange={(e) => setNote(e.target.value)} placeholder={needsReview ? 'Task not fully completed' : 'Reason for this decision'} /></div>}
        {needsValidation && <>
          ...
          <div className="btn-row"><button className="btn btn-primary" onClick={() => act(() => validateSupervisorReport(report.id, correct ? { category, severity, address, note } : { note }), 'Task validated - available in Assign Tasks')}>{correct ? 'Save Correction' : 'Accept / Validate'}</button><button className="btn btn-secondary" onClick={() => setCorrect(!correct)}>{correct ? 'Cancel Correction' : 'Correct'}</button></div>
        </>}
        {view === 'review' && !task && !report.workOrderId && !report.supervisorDismissal && report.status !== 'Resolved' && <button className="btn btn-secondary" onClick={() => act(() => dismissSupervisorReport(report.id, note), 'Report dismissed')}>Reject / Dismiss</button>}
...
export default function SupervisorDashboard({ view = 'dashboard' }) {
  ...
  const [tab, setTab] = useState(view === 'assign' ? 'validated' : view === 'assigned' ? 'assigned' : 'incoming');
  ...
  if (view === 'dashboard') {
    ...
    return <>
      ...
      <div className="grid cols-3">
        <StatCard label="Incoming Tasks" value={queues.incoming.length} to="/supervisor/review" />
        ...
        <StatCard label="Validated Tasks" value={queues.validated.length} to="/supervisor/assign" />
        <StatCard label="Assigned Tasks" value={queues.assigned.length} to="/supervisor/assigned" />
        <StatCard label="In Progress" value={queues.active.length} to="/supervisor/assigned" />
        <StatCard label="Completed" value={queues.completed.length} to="/supervisor/assigned" />
      </div>
    </>;
  }
  ...
  return <>
    <PageHead title={view === 'review' ? 'Review Tasks' : view === 'assign' ? 'Assign Tasks' : 'Assigned Tasks'} sub={view === 'review' ? 'Review citizen and connected infrastructure reports.' : view === 'assign' ? 'Assign validated tasks to existing technicians.' : 'Monitor assigned work and completion.'} />
    ...
    <div className="stack-sm">
      <p className="muted small">{{ incoming: 'Open Review / Validate below a task to inspect its evidence, accept or dismiss it, then use Assign Tasks.', validated: 'Validated reports ready for technician assignment.', assigned: 'Assigned tasks scheduled or en route.', active: 'Technician work in progress, including corrections and repair verification.', completed: 'Completed tasks and supervisor completion reviews.' }[tab]}</p>
      ...
      {list.slice(currentPage * 8, currentPage * 8 + 8).map((entry) => <article className="card" key={entry.kind + entry.item.id}><div className="stack-sm"><div><strong>{entry.item.issue || entry.report.issue || entry.report.ai?.detected || entry.item.category}</strong><div className="btn-row" style={{ marginTop: 8 }}><SourceBadge source={entry.report.source} /><StatusBadge status={entry.item.status} /><SeverityBadge severity={entry.item.priority || entry.item.severity} /></div><p className="muted small">{entry.item.id} · {typeof entry.item.location === 'string' ? entry.item.location : entry.item.location?.address}</p><p className="muted small">Risk: {entry.item.risk} · {fmtDateTime(entry.report.submittedAt)}{entry.item.technician ? ' · ' + entry.item.technician : ''}</p><p>{entry.reason}</p>{entry.kind === 'work' && <p className="muted small">Scheduled: {entry.item.date} {entry.item.time}</p>}
        {entry.kind === 'work' && <p className="muted small">Asset / sensor: {entry.item.assetId || entry.report.assetId || 'Not linked'} · Technician: {entry.item.technician || 'Unassigned'} · Date assigned: {fmtDateTime(entry.item.createdAt)}</p>}</div><button className="btn btn-primary" style={{ alignSelf: 'flex-start', maxWidth: '100%', whiteSpace: 'normal' }} onClick={() => setSelected(entry)}>{tab === 'completed' ? 'Review' : tab === 'validated' ? 'Assign Technician' : tab === 'incoming' ? 'Review / Validate' : 'View Task'}</button></div></article>)}
