  ...
  return <Modal title={task?.id || report.id} onClose={onClose}>
    <div className="stack">
      ...
      <section className="stack-sm"><h3>{view === 'validation' ? 'Data Validation' : 'Supervisor Decision'}</h3>
        {view === 'review' && !task?.technicianId && !report.supervisorDismissal && report.status !== 'Resolved' && !isSupervisorValidated(report) && <button className="btn btn-primary" onClick={() => act(() => acceptSupervisorReport(report.id, note), 'Accepted - available in Data Validation')}>Accept / Validate</button>}
